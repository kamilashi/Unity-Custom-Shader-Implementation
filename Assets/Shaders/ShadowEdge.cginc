if(_EnableDiffraction)
{
    ...
    float3 axis = i.worldCoord.xyz;
    float3 axisDisplacement = float3(1, 1, 1) - i.normal;  // get planar coords
    axisDisplacement *= prerAxisDisplacement;

    float4 shadowCoordShiftedRight = TransformWorldToShadowCoord( axis + axisDisplacement);
    light = GetMainLight(shadowCoordShiftedRight);
    float attenuationShiftedRight = light.shadowAttenuation;

    float4 shadowCoordShiftedLeft = TransformWorldToShadowCoord( axis - axisDisplacement);
    ...
    if (axisDisplacement.x != 0.0f)
    {
        axisDisplacement.x = - 2 * axisDisplacement.x;
    }
    else if(axisDisplacement.y != 0.0f)
    {
        axisDisplacement.y = - 2 * axisDisplacement.y;
    }
    else
    {
        axisDisplacement.z = - 2 * axisDisplacement.z;
    }
        

    shadowCoordShiftedRight = TransformWorldToShadowCoord( axis + axisDisplacement);
    light = GetMainLight(shadowCoordShiftedRight);
    attenuationShiftedRight = light.shadowAttenuation;

    shadowCoordShiftedLeft = TransformWorldToShadowCoord( axis - axisDisplacement);
    light = GetMainLight(shadowCoordShiftedLeft);
